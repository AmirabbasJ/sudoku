    ...
    .map((blockRow, blockRowIndex) =>
      blockRow.map((blocks, blockColIndex) =>
        blocks.map((slots, slotRowIndex) =>
          slots.map((_, slotColIndex) => toId([blockRowIndex, blockColIndex, slotRowIndex, slotColIndex])),
        ),
      ),
    )
    .flat(3);
...
const getBlockIds = (board: Board, id: Id) => {
  ...
  return getAllSlotIds(board)
    .map(idToBoardIndex)
    .filter(([blockRow, blockCol]) => blockRow === selectedBlockRow && selectedBlockCol === blockCol)
    .map(toId);
};
...
const getRowIds = (board: Board, id: Id) => {
  ...
  return getAllSlotIds(board)
    .map(idToBoardIndex)
    .filter(([blockRow, __, slotRow]) => selectedBlockRow === blockRow && slotRow === selectedSlotRow)
    .map(toId);
};
...
const getColIds = (board: Board, id: Id) => {
  ...
  return getAllSlotIds(board)
    .map(idToBoardIndex)
    .filter(([___, blockCol, ____, slotCol]) => selectedBlockCol === blockCol && slotCol === selectedSlotCol)
    .map(toId);
};
...
export const getCoveredSlotIds = R.pipe(R.juxt([getBlockIds, getColIds, getRowIds]), R.flatten);
...
export const getMutableSlotIds = (board: Board) => getAllSlotIds(board).filter(id => getSlot(board, id) === '');
